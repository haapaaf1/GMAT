...
//------------------------------------------------------------------------------
// Rvector3 GetUnitVector(const Real &er, const Real &flattening) const
//------------------------------------------------------------------------------
Rvector3 LatLonHgt::GetUnitVector(const Real &er, const Real &flattening)
{
    ...
    if (type == "Geodetic")
    {
	GeodeticToGeocentricLat(er,flattening);
	unit[0] = GmatMathUtil::Cos(latitude) * GmatMathUtil::Cos(longitude);
	unit[1] = GmatMathUtil::Cos(latitude) * GmatMathUtil::Sin(longitude);
	unit[2] = GmatMathUtil::Sin(latitude);
	GeocentricToGeodeticLat(er,flattening);
    }
    ...
    {
	ReducedToGeocentricLat(er,flattening);
	unit[0]    = GmatMathUtil::Cos(latitude) * GmatMathUtil::Cos(longitude);
	unit[1]    = GmatMathUtil::Cos(latitude) * GmatMathUtil::Sin(longitude);
	unit[2]    = GmatMathUtil::Sin(latitude);
	GeocentricToReducedLat(er,flattening);
    }
...
//------------------------------------------------------------------------------
// Real SatCartesianToLatLonHgt(const Rvector3& cartVector,
//                         const Real &equatorialRadius, const Real &flattening,
//			 const std::string typ, const std::string &hgtReference)
//
/**
...
void LatLonHgt::SatCartesianToLatLonHgt( const Rvector3 &cartVector, 
			        ...
			        const std::string &hgtReference)
{
  ...
  // Get position (X, Y, Z)
  Real posX = cartVector.Get(0); 
  Real posY = cartVector.Get(1);
  Real posZ = cartVector.Get(2);
...
//------------------------------------------------------------------------------
// Real CartesianToLatLonHgt(const Rvector3& position,
//                         const Real &equatorialRadius, const Real &flattening,
//			 const std::string typ)
//
/** This routine will compute the geodetic latitude, longitude, and height
			 ...
			 const std::string &hgtReference)
{
  ...
  // Get geodetic latitude measured north from the equator
  Real rDelta = GmatMathUtil::Sqrt(posX*posX+posY*posY);
  Real latGD = GmatMathUtil::ATan2(posZ,rDelta*(1-eccentricity*eccentricity)); 

  // Compute sin and cos of the geodetic latitude for convenience
  Real slat = GmatMathUtil::Sin(latGD);
  Real clat = GmatMathUtil::Cos(latGD);

  // C is the curvature in the meridian
  Real C = equatorialRadius/GmatMathUtil::Sqrt(1-eccentricity*eccentricity*slat*slat);

  // This is the height above the reference ellipsoid
  Real h_ellp = rDelta/clat-C;
