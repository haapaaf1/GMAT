C IRISTORM.FOR ------------------------------------21 JANUARY 1996
C
C*****************************************************************
C********* INTERNATIONAL REFERENCE IONOSPHERE (IRI-1990),
C********* UPDATING FOR EFFECTS OF LARGE SCALE TIDs
C********* DURING SUBSTORMS AND SC"s & SEVERE MAGNETIC STORMS.
C********* IZMIRAN, Russion Academy of Sciences,
...
C***    - MONTH-DAY OR DAY OF YEAR                             ***
C***    - LOCAL OR UNIVERSAL TIME                              ***
C***    - GIVEN NUMBER OF SUBSTORS                             ***
C***    - MAXIMUM AE-INDICES,REACHED IN SUBSTORMS              ***
C***    - LOCAL OR UNIVRSAL TIME OF SUBSTORM"S ONSETS          ***
C***    - YOUR CHOICE OF VARIABLES AND OPTIONS                 ***
C*****************************************************************
...
C***    - OPTION TO USE GULYAEVA'S B0TTOMSIDE THICKNESS PARA.  ***
...
C---------LSTID-----------------------------
      INTEGER ICEZ
      REAL AEM(10),UTS70M(10),DFD(10),DHMD(10)
      INTEGER JDS(10),JMS(10),JYS(10),NDAYS(10)
C--------STORM  -----------------------------
        REAL AP(7)
C--------------------------------------------
      CHARACTER*2	MTENE(3)
     ...
     &           IPARI(7)
      ...
      LOGICAL		PIKTAB,NOTBEG,STORE,NODEN,NOTEM,NOION,TENEOP
      LOGICAL           OLD79,TOPSI,BOTTO,BELOWE,HEIPRO
     ...
     &        ' DIP ','MODIP','F10.7',' SZA '/
      DATA  JMAG,LATI,LONGI,MLAT,MLONG	/0,55.1,037.0,45.1,293.1/,
     &      XHIGHT,R,MONTH,MMDD,DHOUR	/100.,100.,10,0315,27.0/,
     &      IVAR,BVAR,EVAR,SVAR		/6,100.,1000.,100./,
     &      IRIPS,ISELEC,IJFOF2,IJHMF2    /1,2,2,1/,
     &      JAGNR,JAGNRM,MONTH0,RG0  	/0,1,0,0.0/
...
5100  FORMAT(1X/////8X,'**** INTERNATIONAL REFERENCE ',
     &	'IONOSPHERE 1990 ****'/1X,
     &'UPDATING FOR EFFECTS OF THE LARGE SCALE IGW DURING SUBSTORMS'/,
     &63('*')/' THIS PROGRAM ALLOWS YOU',
...
5602  FORMAT(1X//' **** WHICH PARAMETER DO YOU WANT TO ',
     ...
     & ' 5  MONTH,DAY  SUBSTORMS,STORMS #',I2,I3,'#',7X,
     &'11  F PEAK VALUES, AE, TS70  '
     & /' 6  ',A5,
...
C-------------LSTID OR STORM ----------------------
	WRITE(MONITO,*) 'TO UPDATE IRI WITH RESPECT TO SUBSTORMS'
	WRITE(MONITO,*) 'AND SC OF STORMS:     TYPE  1   ELSE  0'
	READ(EGNR,*)ISUBST
	WRITE(MONITO,*) 'TO UPDATE IRI WITH RESPECT TO STORMS'
	WRITE(MONITO,*) 'TYPE  2   ELSE  0'
	READ(EGNR,*)ISTORM
	IF(ISUBST.NE.0 .OR. ISTORM.NE.0)THEN
	WRITE(MONITO,*) 'INPUT YEAR    #1970#'
	READ(MONITO,*)IYEAR

        ICEZ=1
        IF(MONTH.GE.5 .AND. MONTH.LE.9)ICEZ=2
	END IF
C------------------------------------------
...
8136  READ(EGNR,*,ERR=8137,END=3330) XHIGHT
      IF((XHIGHT.LE.XMAX).AND.(XHIGHT.GE.XMIN0)) GOTO 8138
...
6709   CONTINUE
	IF(ISUBST .EQ.0)GOTO 4115
      WRITE(MONITO,*) 'INPUT PARAMETERS FOR UPDATING DURING SUBSTORMS:'
      WRITE(MONITO,*) '(method is workable now only for durk time)'
      WRITE(MONITO,*) '(from sunset to sunrise.                  )'
      WRITE(MONITO,*) '********************************************'
      WRITE(MONITO,*) 'INPUT INS -a number of substorms'
      WRITE(MONITO,*) 'It is recommended only substorms during '
      WRITE(MONITO,*)  'or less than 5 hours before given local time :'
      WRITE(MONITO,*)  '                                         # 1 #'
        READ(EGNR,*)INS
        IF(INS.GT.0)THEN
        DO 4603 I=1,10
...
4603    CONTINUE
        ...
        WRITE(MONITO,*) 'Input for each substorm:'
        WRITE(MONITO,*) '    day, month, year, UT of onset and'
        WRITE(MONITO,*) '    (AE)m- maximum value of AE-index, reached' '
        WRITE(MONITO,*) '    during substorm.'
        WRITE(MONITO,*) '                  # 15 03 1970 0.1 1000.0 #'
        WRITE(MONITO,*) '********************************************'
        WRITE(MONITO,*) '    '

	DO I=1,INS
        WRITE(MONITO,*) ' For the substorm of number ',I,' :'
	READ(MONITO,*) JDS(I),JMS(I),JYS(I),UTS70M(I),AEM(I)
        WRITE(MONITO,*) '*************************************'
	END DO
4106	FORMAT(3(I2,1X),F5.2,1X,F7.2)
	END IF
4115    CONTINUE
	IF(ISTORM .EQ.0)GOTO 4116
      WRITE(MONITO,*) 'INPUT PARAMETERS FOR UPDATING DURING STORMS:'
      WRITE(MONITO,*) '(method is workable only for AP-index > 6.6)'
      WRITE(MONITO,*) '********************************************'
      WRITE(MONITO,*) '     '
      WRITE(MONITO,*) 'F107- DAILY F10.7 RADIO FLUX FOR PREVIOUS DAY'
      WRITE(MONITO,*) 'F107M- 3 MONTH AVERAGE F10.7 SOLAR RADIO FLUX '
      WRITE(MONITO,*) '                                  # 100 110 #'
      WRITE(MONITO,*) '********************************************'
      WRITE(MONITO,*) '     '
      READ(MONITO,*) F107,F107M
      WRITE(MONITO,*) '********************************************'
      WRITE(MONITO,*) 'AP(1:7)       A SET OF AP-INDICES'
      WRITE(MONITO,*) 'AP(1)- DAILY AP'
      WRITE(MONITO,*) 'AP(2)- 3 HOUR AP FOR CURRENT TIME'
      WRITE(MONITO,*) 'AP(3)- 3 HOUR AP FOR 3 HOURS BEFORE CURRENT TIME'
      WRITE(MONITO,*) 'AP(4)- 3 HOUR AP FOR 6 HOURS BEFORE CURRENT TIME'
      WRITE(MONITO,*) 'AP(5)- 3 HOUR AP FOR 9 HOURS BEFORE CURRENT TIME'
      WRITE(MONITO,*) 'AP(6)- AVERAGE OF EIGHT 3 HOUR AP INDICES       '
      WRITE(MONITO,*) '       FROM 12 TO 33 HRS PRIOR TO CURRENT TIME  '
      WRITE(MONITO,*) 'AP(7)- AVERAGE OF EIGHT 3 HOUR AP INDICES       '
      WRITE(MONITO,*) '       FROM 36 TO 59 HRS PRIOR TO CURRENT TIME  '
      WRITE(MONITO,*) '       #50.0 50.0 50.0 50.0 50.0 50.0 50.0 50.0#'
      WRITE(MONITO,*) '************************************************'
      READ(MONITO,*)(AP(IAP),IAP=1,7)


C-------------------------------------------------------
4116       IF(NOTBEG) GOTO 3293
c
c Te = f ( Ne )  option ..........................................
c
...
5300    TNOPIV=((TENEOP).AND.(IVAR.EQ.6))
      write(*,*) '* Updating IRI parameters are being calculated *'
      if(NODEN) goto 2889
...
c
	...
	IF(ITEVA.GT.11) ITEVA=11
      PIKTAB=((XHIGHT.LT.1.0).AND.(IVAR.NE.6))
      IF(PIKTAB) THEN
...
8192  FORMAT(//////////3X,A5,5X,'PEAK ALTITUDES IN KM',12X,'PEAK DEN',
     &  'SITIES IN CM-3'/4X,A4,'   HMF2 HMF1   B0  HME  HMD     NMF2',
     &  '   NMF1   NVBot   NME     NMD')
...
C
	...
	if(ivar.eq.3) call moda(0,month,iday,daynr)
C----LSTID OR STORM  ---------------------

	IF(ISUBST.NE.0 .OR. ISTORM.NE.0)THEN
	CALL DAYRES(IDAY,MONTH,IYEAR,IDS,IMS,IYS)
	CALL NNDAY(IDS,IMS,IYS,DAYNR)
	END IF
C----------------------------------
...
C----- LSTID  --------------------------
	IF(ISUBST .NE. 0)THEN
	DO I=1,INS
	CALL DAYRES(JDS(I),JMS(I),JYS(I),IDS,IMS,IYS)
	CALL NNDAY(IDS,IMS,IYS,NDAYS(I))
	END DO
C	WRITE(*,*)'NDAYS:',(NDAYS(I),I=1,INS)
	END IF
C----------------------------------------
...
C
        WRITE(FILENA,104) MONTH+10
1344	lread=1
c        OPEN(IUCCIR,FILE=FILENA,STATUS='OLD',FORM='BINARY')
        OPEN(IUCCIR,FILE=FILENA,STATUS='OLD',FORM='UNFORMATTED')
        READ(IUCCIR) XXXZZ,F2,FM3
...
C-------LSTID ---------------------------
          ...
          IF(IJFOF2 .EQ. 0)GOTO 3700
 	  IF(ISUBST .EQ. 0)GOTO 3701

          FLSTID=0.
          HLSTID=0.

          IF(INS.EQ.0 )GOTO 4610
          DO 4609 I=1,10
          DFD(I)=0.
          DHMD(I)=0.
4609      CONTINUE
C	WRITE(*,*)'NDAY=',DAYNR,' NDAYS=',NDAYS
          DO 4608 I=1,INS
          CALL LSTID(MLAT,LONGI,ICEZ,R,AEM(I),HOUR,DAYNR,NDAYS(I),
     &    UTS70M(I),SAX,SUX,DFD(I),DHMD(I))
          FLSTID=FLSTID+DFD(I)
          HLSTID=HLSTID+DHMD(I)
4608      CONTINUE
4610      FOF2=FOF2+FLSTID
          HMF2=HMF2+HLSTID
          NMF2=1.24E10*FOF2*FOF2
          WRITE(*,*)I,' FLSTID=',FLSTID,' HLSTID=',HLSTID
3701	  CONTINUE

C------STORM  ---------------------------------------
	IF(ISTORM.EQ.0)GOTO 3700
        IF(AP(2).LE.6.6)GOTO 3700

        IF(F107.LT.0. .AND. F107M.LT.0.)THEN
        F107=COV
        F107M=COV
        END IF
        call ddfh(DAYNR,HOUR,UT,LATI,LONGI,F107M,F107,AP,DFOF2N,DHMF2)
        write(*,*)'storm  dfof2n=',dfof2n,' dhmf2=',dhmf2
C--CURRENT UPD  ------------------------------
        FOF2=FOF2*(1.+dfof2n)
        HMF2=HMF2 + dhmf2
        NMF2=1.24E10*FOF2*FOF2
...
C------------------------------------------------------------
	...
	IF((.not.HEIPRO).and.(.not.PIKTAB)) THEN
...
c
	...
	IF(F1REG) IHMF1=INT(HMF1+.5) 
        WRITE(AGNR,3910) XCOR,INT(HMF2+.5),IHMF1,INT(B0+.5),
     &    INT(HME+.5),INT(HMD+.5),INT(NMF2/1.E6+.5),
     &    INT(PNMF1/1.E6+.5),INT(VNER/1.E6+.5),INT(NME/1.E6+.5),
     &    INT(NMD/1.E6+.5)
        IF(STORE) WRITE(IUOUT,3910) XCOR,INT(HMF2+.5),
     &    IHMF1,INT(B0+.5),INT(HME+.5),INT(HMD+.5),
     &    INT(NMF2/1.E6+.5),INT(PNMF1/1.E6+.5),INT(VNER/1.E6+.5),
     &    INT(NME/1.E6+.5),INT(NMD/1.E6+.5)
3910    FORMAT(1X,F7.1,2X,5I5,3X,5I7)
        GOTO 2135
      ENDIF
c
c normal output
...
C-------LSTID + STORM --------------------------------
	IF(ISUBST.NE.0 .OR. ISTORM.NE.0)THEN
     ...
     &	     XHI,MLAT,MLONG,DIP,COV,DAYNR,UT,SUNDEC,FOF2,FLSTID,
     &       HMF2,HLSTID,DFOF2N,DHMF2
	ELSE
      WRITE(AGNR,2293) LATI,LONGI,HEIGHT,R,MONTH,IDAY,HOUR,
     &	     XHI,MLAT,MLONG,DIP,COV,DAYNR,UT,SUNDEC,FOF2,HMF2
	END IF
C---------------------------------------------
...
C--------LSTID +STORM  --------------------------------
	...
	ELSE
        ...
        WRITE(IUOUT,2293) LATI,LONGI,HEIGHT,R,MONTH,IDAY,HOUR,
     &	   XHI,MLAT,MLONG,DIP,COV,DAYNR,UT,SUNDEC,FOF2,HMF2
	END IF
6193  FORMAT(1X,'I-',A5,'--',A5,'-',A3,'-',A5,'-I',11('-'),A4,
     &  3A2,'--I',11('-'),A3,9('-'),'I')
6196  FORMAT(1X,10('-'),A5,'-----',A5,14('-'),A5,30('-'))

2193  FORMAT(' LAT/LON=',F5.1,'/',F5.1,'    H=',F6.1,
     &  '  RZ12=',F5.1,' MMDD:',I2,I2,'  LT:',F4.1,'  SZA=',F5.1/
     &  ' MLA/MLO=',F5.1,'/',F5.1,'  DIP=',F5.1,'  F10.7=',F5.1,
     &  '  DDD:',I4,'  UT:',F4.1,'  SDE=',F5.1,/,
     &  '  foF2=',F5.2,'  DfoF2SUB=',F5.2,'  hmF2=',F6.2,
     &  '  DhmF2SUB=',F6.2,'DFSTORM=',F5.2,' DHSTORM=',F6.2,
     &  /1X,74('-'))
2293  FORMAT(' LAT/LON=',F5.1,'/',F5.1,'    H=',F6.1,
     &  '  RZ12=',F5.1,' MMDD:',I2,I2,'  LT:',F4.1,'  SZA=',F5.1/
     &  ' MLA/MLO=',F5.1,'/',F5.1,'  DIP=',F5.1,'  F10.7=',F5.1,
     &  '  DDD:',I4,'  UT:',F4.1,'  SDE=',F5.1,/,
     &  '  foF2=',F5.2,'  hmF2=',F6.2,
