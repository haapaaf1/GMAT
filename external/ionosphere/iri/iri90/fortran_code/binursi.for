C
C The program BINURSI.FOR generates the URSI coefficient files
...
1000    format(1x,'which month (0 for all month)?')
	...
	mo=mmo+10
	write(6,1001) mo,mo
1001	format(1x,'Generating URSI',I2,'.ASC from URSI',i2,'.BIN')	
	write(filename,104) mo
104	format('ursi',i2,'.bin')
	open(10,file=filename,status='old',form='unformatted')
	read(10) f2
	write(filename,1104) mo
1104	format('ursi',i2,'.asc')
