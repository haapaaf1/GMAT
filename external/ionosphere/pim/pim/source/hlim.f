...
C
      ...
      REAL ZD(100),NED(100),EOUT(100)
      REAL FOUT(100),TECALT(NTEC),POUT(100)
...
C
        DO IL = 1, NTEC
         FOUT(IL) = EXP(FOUT(IL))
         NED(IL) = LOG(MAX(EXP(EOUT(IL))+FOUT(IL)+POUT(IL),1.E-9))
        ENDDO
       ELSE
C
...
C     RE     Real       km              The mean radius of the Earth
C
      REAL RE,A1,A2,A3,A4,A5,A7
      PARAMETER(RE=6375.,A1=1.4,A2=1.53,A3=-.036,A4=30.76,A5=159.9,
     &          A7=6.27)
C
C  Input variables
C
...
C  Local variables
C
      REAL PI,DTOR,HTOR,COSLAM,COSLA2,L,F,X,A6,G,A8,A9,A9M1,H
C
C  Calculate pi
C
      PI=4.*ATAN(1.)
