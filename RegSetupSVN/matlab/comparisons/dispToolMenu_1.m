...
availFolders = dir('*_reports');

% Check to see if STK, FF, and Exact report folders exist inside the Good_reports folder
if isdir([cd,'/Good_reports/STK']);
    eval(['availFolders(',num2str(size(availFolders,1)+1),').name = ''STK'';']);
end;
if isdir([cd,'/Good_reports/FF']);
    eval(['availFolders(',num2str(size(availFolders,1)+1),').name = ''FF'';']);
end;
% if isdir([cd,'/Good_reports/Exact']);
%     eval(['availFolders(',num2str(size(availFolders,1)+1),').name = ''Exact'';']);    
% end;
