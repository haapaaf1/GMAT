%$Id: StopTargetWhile.m,v 1.8 2008/04/18 13:42:11 lojun Exp $ 
...
GMAT stopReport.Filename = ./output/SystemTest/StopTargetWhile.report;
...
EndWhile;
Target DefaultDC;
   Vary DefaultDC(Burn1.Element1 = 2.5, {Perturbation = 0.0001, MaxStep = 0.2, Lower = 0, Upper = 3.14159, AdditiveScaleFactor = 0, MultiplicativeScaleFactor = 1});
   Maneuver Burn1(Sat)
   Propagate DefaultProp(Sat) {Sat.Earth.Apoapsis};
   Report stopReport Sat.ElapsedSecs Sat.SMA Sat.ECC Sat.INC Sat.RAAN ...
       Sat.AOP Sat.TA Sat.BetaAngle;
   While Sat.TA < 179.999999999
      Stop;
   EndWhile
   Achieve DefaultDC(Sat.Earth.RMAG = 42165.0, {Tolerance = 0.1});
EndTarget;  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC  % For targeter DefaultDC
