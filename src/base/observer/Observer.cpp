//$Id: Observer.cpp 5364 2008-04-10 21:50:04Z lindajun $
//------------------------------------------------------------------------------
//                                  Observer 
//------------------------------------------------------------------------------
...
// 
/**
 * Implements the Observer base class.
 *    Observer internal state is in EarthMJ2000Eq Cartesian.
...
//#define DEBUG_OBSERVER 1 
//#define DEBUG_OBSERVER_SET 1 
//#define DEBUG_OBSERVER_CS 1 
...
// Observer parameter types
const Gmat::ParameterType   
Observer::PARAMETER_TYPE[ObserverParamCount - ObserverObjectParamCount] = 
   {
      ...
      Gmat::OBJECTARRAY_TYPE, // Sensors
      Gmat::OBJECT_TYPE,      // Attitude 
   };
   
const std::string 
Observer::PARAMETER_LABEL[ObserverParamCount - ObserverObjectParamCount] = 
   {
      "Epoch",
      "Element1", 
      "Element2", 
...
/*
const std::string Observer::MULT_REP_STRINGS[EndMultipleReps - CART_X] = 
{
  if (isGroundBased) {
    // Cartesian
    "X",
...
const Integer Observer::ATTITUDE_ID_OFFSET = 20000;
 ...
 *
 */
Observer::Observer(const std::string &name, const std::string &typeStr) :
   ObserverObject          (Gmat::OBSERVER, typeStr, name),
   ...
   csSet                (false)
{
   //MessageInterface::ShowMessage("=====> Observer::Observer(%s) entered\n",
   //                              name.c_str());
   
   objectTypes.push_back(Gmat::OBSERVER);
   objectTypeNames.push_back("Observer");
   ...
   parameterCount = ObserverParamCount;
   
   BuildElementLabelMap();
   
   //MessageInterface::ShowMessage("=====> Observer::Observer(%s) exiting\n",
   //                              name.c_str());
}
...
Observer::~Observer()
{
   ...
   for (ObjectArray::iterator i = sensors.begin(); i < sensors.end(); ++i)
      delete *i;
...
//---------------------------------------------------------------------------
/**
 * Copy Constructor for base Observer structures.
 *
 ...
 */
Observer::Observer(const Observer &a) :
   ObserverObject          (a),
...
//---------------------------------------------------------------------------
/**
 * Assignment operator for Observer structures.
 * 
 ...
 * 
 * @todo Determine how to handle hardware when copying Observer objects.
 */
Observer& Observer::operator=(const Observer &a)
{
   // Don't do anything if copying self
   if (&a == this)
      return *this;

   ObserverObject::operator=(a);
...
//---------------------------------------------------------------------------
CoordinateSystem* Observer::GetInternalCoordSystem()
{
...
//---------------------------------------------------------------------------
void Observer::SetInternalCoordSystem(CoordinateSystem *cs)
{
   #if DEBUG_OBSERVER_CS
      MessageInterface::ShowMessage
         ("Observer::SetInternalCoordSystem() cs=%s<%p> on %s\n",
 ...
 */
//---------------------------------------------------------------------------
void Observer::SetState(const Rvector6 &cartState)
{
   #if DEBUG_OBSERVER_SET
      MessageInterface::ShowMessage("Observer::SetState(Rvector6)\n");
      MessageInterface::ShowMessage(
      "Observer::SetState(Rvector6) cartesianState=%s\n",
 ...
 */
//---------------------------------------------------------------------------
void Observer::SetState(const std::string &elementType, Real *instate)
{
   #if DEBUG_OBSERVER_SET
      MessageInterface::ShowMessage(
         "Observer::SetState() elementType = %s, instate =\n"
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetState(const Real s1, const Real s2, const Real s3, 
                          const Real s4, const Real s5, const Real s6)
{
    state[0] = s1;
    state[1] = s2;
...
//------------------------------------------------------------------------------
/**
 * "Unhide" the ObserverObject method.
 * 
 * @return the core PropState.   
 */
//------------------------------------------------------------------------------
PropState& Observer::GetState()
{
   return ObserverObject::GetState();
}
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetState(std::string rep) 
{
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetState(Integer rep) 
{
   rvState = GetStateInRepresentation(rep);
   return rvState;
}
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetCartesianState() 
{   
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetKeplerianState() 
{
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetModifiedKeplerianState() 
{
//   rvState = stateConverter.Convert(state.GetState(),stateType,
...
/*
Anomaly Observer::GetAnomaly() const
{
   return trueAnomaly;
}
*/

Rmatrix33 Observer::GetAttitude(Real a1mjdTime) const
{
   if (attitude) return attitude->GetCosineMatrix(a1mjdTime);
   else 
   {
      //MessageInterface::PopupMessage(Gmat::WARNING_, 
      //"No attitude defined for observer %s, returning identity matrix.\n",
      //instanceName.c_str());
...
Rvector3  Observer::GetAngularVelocity(Real a1mjdTime) const
{
   if (attitude) return attitude->GetAngularVelocity(a1mjdTime);
   else 
   {
      //MessageInterface::PopupMessage(Gmat::WARNING_, 
      //"No attitude defined for observer %s, returning zero angular velocity vector.\n",
      //instanceName.c_str());
      return Rvector3(); // temporary - return zero vector
   }
}

UnsignedIntArray Observer::GetEulerAngleSequence() const
{
   if (attitude) 
   ...
   {
      std::string errmsg = 
         "Error attempting to retrieve Euler Angle Sequence for observer \"";
      errmsg += instanceName + "\", for which no attitude has been set.\n";
      throw ObserverObjectException(errmsg);
   }
}
...
//------------------------------------------------------------------------------
/**
 * This method returns a clone of the Observer.
 *
 * @return clone of the Observer.
 *
 */
//------------------------------------------------------------------------------
GmatBase* Observer::Clone() const
{
   Observer *clone = new Observer(*this);
 ...
 */
//---------------------------------------------------------------------------
void Observer::Copy(const GmatBase* orig)
{
   operator=(*((Observer *)(orig)));
}
...
//---------------------------------------------------------------------------
bool Observer::RenameRefObject(const Gmat::ObjectType type,
                                 const std::string &oldName,
                                 const std::string &newName)
{
   #if DEBUG_RENAME
   MessageInterface::ShowMessage
      ("Observer::RenameRefObject() type=%s, oldName=%s, newName=%s\n",
 ...
 */
std::string Observer::GetRefObjectName(const Gmat::ObjectType type) const
{
   ...
   return ObserverObject::GetRefObjectName(type);
}
 ...
 */
//------------------------------------------------------------------------------
const ObjectTypeArray& Observer::GetRefObjectTypeArray()
{
...
//------------------------------------------------------------------------------
const StringArray& 
      Observer::GetRefObjectNameArray(const Gmat::ObjectType type)
{
   ...
   return ObserverObject::GetRefObjectNameArray(type);
}
...
//------------------------------------------------------------------------------
bool Observer::SetRefObjectName(const Gmat::ObjectType type, const std::string &name)
{
   if (type == Gmat::COORDINATE_SYSTEM)
   {
      coordSysName = name;
   ...
   return ObserverObject::SetRefObjectName(type, name);
}
 ...
 */
GmatBase* Observer::GetRefObject(const Gmat::ObjectType type, 
                                   const std::string &name)
{
   ...
   switch (type)
   {
      ...
      case Gmat::ATTITUDE:
         ...
         MessageInterface::ShowMessage(
         "In Observer::GetRefObject - returning Attitude poinetr <%p>\n", attitude);
         #endif
         //MessageInterface::ShowMessage("CoordinateSystem named %s\n", name.c_str());
         return attitude;
      ...
      case Gmat::SENSOR:
         ...
         for (ObjectArray::iterator i = sensors.begin(); 
              i < sensors.end(); ++i) {
            if ((*i)->GetName() == name)
               return *i;
         }
   ...
   return ObserverObject::GetRefObject(type, name);
}
...
//                   const std::string &name)
//------------------------------------------------------------------------------
bool Observer::SetRefObject(GmatBase *obj, const Gmat::ObjectType type, 
                              const std::string &name)
{
   #ifdef DEBUG_SC_REF_OBJECT
   MessageInterface::ShowMessage("Entering Observer::SetRefObject\n");
   #endif
   ...
   else if (type == Gmat::COORDINATE_SYSTEM)
   {
      ...
      #if DEBUG_OBSERVER_CS
      MessageInterface::ShowMessage
         ("Observer::SetRefObject() coordinateSystem=%s<%p>, cs=%s<%p> on %s\n",
      ...
      if (coordinateSystem != cs)
      {
         ...
         #if DEBUG_OBSERVER_CS
         MessageInterface::ShowMessage
            ("Observer::SetRefObject() coordinateSystem applied ----------\n");
         #endif
      }
   ...
   {
      #ifdef DEBUG_SC_ATTITUDE
         MessageInterface::ShowMessage("Setting attitude object on observer %s\n",
         instanceName.c_str());
      #endif
      ...
      // set epoch ...
         ...
         instanceName.c_str());
         MessageInterface::ShowMessage(
         "Setting epoch on attitude object for observer %s\n",
         instanceName.c_str());
      #endif
   ...
   #ifdef DEBUG_SC_REF_OBJECT
   MessageInterface::ShowMessage
      ("Exiting Observer::SetRefObject, Calling ObserverObject::SetRefObject()\n");
   #endif
   
   return ObserverObject::SetRefObject(obj, type, name);
}
 ...
 */
ObjectArray& Observer::GetRefObjectArray(const Gmat::ObjectType type)
{
   if (type == Gmat::FUEL_TANK)
      return tanks;
   if (type == Gmat::THRUSTER)
      return thrusters;
   return ObserverObject::GetRefObjectArray(type);
}
 ...
 * @return Reference to the array.
 */
ObjectArray& Observer::GetRefObjectArray(const std::string& typeString)
{
   if ((typeString == "FuelTank") || (typeString == "Tanks"))
      return tanks;
   if ((typeString == "Thruster") || (typeString == "Thrusters"))
      return thrusters;
   return ObserverObject::GetRefObjectArray(typeString);
}
 ...
 */
Integer Observer::GetParameterID(const std::string &str) const
{
   #ifdef DEBUG_PARM_PERFORMANCE
      MessageInterface::ShowMessage("Observer::GetParameterID(%s)\n", str.c_str());
   #endif
   #ifdef DEBUG_GET_REAL
   MessageInterface::ShowMessage("In Observer::GetParameterID, str = %s\n ",
   str.c_str());
   #endif
   ...
   for (Integer ii = 0; ii < sz; ii++)
      if (str == MULT_REP_STRINGS[ii])
      {
         #ifdef DEBUG_GET_REAL
         MessageInterface::ShowMessage(
         "In Observer::GetParameterID, multiple reps found!! - str = %s and id = %d\n ",
         str.c_str(), (ii + CART_X));
         #endif
         return ii + CART_X;
      }
   ...
   #ifdef DEBUG_GET_REAL
   MessageInterface::ShowMessage(
   "In Observer::GetParameterID, after checking for elements, id = %d\n ",
   retval);
   #endif
   if (retval != -1) return retval;
   
   for (Integer i = ObserverObjectParamCount; i < ObserverParamCount; ++i)
   {
      if (str == PARAMETER_LABEL[i - ObserverObjectParamCount])
      {
         #ifdef DEBUG_OBSERVER_SET
         MessageInterface::ShowMessage(
         "In Observer::GetParameterID, setting id to %d for str = %s\n ",
         i, str.c_str());
         #endif
         return i;
      }
   ...
   return ObserverObject::GetParameterID(str);
}
 ...
 */
//---------------------------------------------------------------------------
bool Observer::IsParameterReadOnly(const Integer id) const
{
   ...
   // Hide ObserverObject epoch so observer can treat it as a string   
   if (id == EPOCH_PARAM)
   {
      return true;
   }
   ...
   return ObserverObject::IsParameterReadOnly(id);
}
 ...
 */
//---------------------------------------------------------------------------
bool Observer::IsParameterReadOnly(const std::string &label) const
{
   return IsParameterReadOnly(GetParameterID(label));
}


std::string Observer::GetParameterText(const Integer id) const
{
   #ifdef DEBUG_SC_PARAMETER_TEXT
   MessageInterface::ShowMessage("Observer::GetParameterText - called with id = %d\n",
   id);
   #endif
   if ((id >= CART_X) && (id < EndMultipleReps))
   {
      #ifdef DEBUG_SC_PARAMETER_TEXT
      MessageInterface::ShowMessage("Observer::GetParameterText - returning text = %s\n",
   ...
   if ((id >= ObserverObjectParamCount) && (id < ObserverParamCount))
      return PARAMETER_LABEL[id - ObserverObjectParamCount];
   ...
   #ifdef DEBUG_SC_PARAMETER_TEXT
   MessageInterface::ShowMessage(
   "Observer::GetParameterText - calling through to base class .....\n");
   #endif
   return ObserverObject::GetParameterText(id);
}
 ...
 */
//------------------------------------------------------------------------------
Gmat::ParameterType Observer::GetParameterType(const Integer id) const
{
   if ((id >= CART_X) && (id < EndMultipleReps))
      return Gmat::REAL_TYPE;
   if ((id >= ObserverObjectParamCount) && (id < ObserverParamCount))
      return PARAMETER_TYPE[id - ObserverObjectParamCount];
   if (id >= ATTITUDE_ID_OFFSET)
      if (attitude) 
         return attitude->GetParameterType(id - ATTITUDE_ID_OFFSET);

    return ObserverObject::GetParameterType(id);
}
 ...
 */
//------------------------------------------------------------------------------
std::string Observer::GetParameterTypeString(const Integer id) const
{
//    return ObserverObject::PARAM_TYPE_STRING[GetParameterType(id)];
 ...
 */
Real Observer::GetRealParameter(const Integer id) const
{
   #ifdef DEBUG_GET_REAL
      MessageInterface::ShowMessage(
      "In Observer::GetReal, asking for parameter %d, whose string is \"%s\"\n", 
      ...
      (id >= CART_X      && id < EndMultipleReps))
   {
      #ifdef DEBUG_GET_REAL
         MessageInterface::ShowMessage(
         "In Observer::GetReal, calling GetElement ....... \n");
      #endif
      return (const_cast<Observer*>(this))->GetElement(GetParameterText(id));
   }
   ...
   return ObserverObject::GetRealParameter(id);
}
 ...
 * @return The parameter's value.
 */
Real Observer::GetRealParameter(const std::string &label) const
{
   // Performance!
    if (label == "A1Epoch")
 ...
 */
Real Observer::SetRealParameter(const Integer id, const Real value)
{
   #ifdef DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage("In Observer::SetRealParameter, id = %d and value = %.12f\n",
   id, value);
   #endif
   if (id >= CART_X && id < EndMultipleReps)
   {
   ...
   return ObserverObject::SetRealParameter(id, value);
}
 ...
 *         parameter type is not Real.
 */
Real Observer::SetRealParameter(const std::string &label, const Real value)
{
   #ifdef DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage(
   "In Observer::SetRealParameter(label), label = %s and value = %.12f\n",
   label.c_str(), value);
   #endif
   ...
   */
   return ObserverObject::SetRealParameter(label, value);
}
 ...
 */
std::string Observer::GetStringParameter(const Integer id) const
{
    if (id == SC_EPOCH_ID)
       return scEpochStr;
    ...
    return ObserverObject::GetStringParameter(id); 
}
 ...
 *         is no string association.
 */
std::string Observer::GetStringParameter(const std::string &label) const
{
//   if (label == "StateType")
//      return stateType;
//   
 ...
 *         StringArray.
 */
const StringArray& Observer::GetStringArrayParameter(const Integer id) const
{
   if (id == FUEL_TANK_ID)
      return tankNames;
   if (id == THRUSTER_ID)
   ...
   return ObserverObject::GetStringArrayParameter(id);
}
 ...
 */
bool Observer::SetStringParameter(const Integer id, const std::string &value)
{
   #ifdef DEBUG_SC_SET_STRING
      MessageInterface::ShowMessage
         ("Observer::SetStringParameter() string parameter %d (%s) to %s\n", 
   ...
   if ((id < ObserverObjectParamCount) || (id >= ObserverParamCount))
      return ObserverObject::SetStringParameter(id, value);
   ...
   //   throw ObserverObjectException("Parameter " + GetParameterText(id) +
   ...
   {  
      ...
      {   
         throw ObserverObjectException("Unknown state element representation: " + 
            value);
      }
      #ifdef DEBUG_SC_SET_STRING
      MessageInterface::ShowMessage("Observer::SetString - setting display state type to %s\n",
      value.c_str());
      #endif
   ...
   #ifdef DEBUG_SC_SET_STRING
   MessageInterface::ShowMessage
      ("Observer::SetStringParameter() returning true\n");
   #endif
 ...
 */
//---------------------------------------------------------------------------
bool Observer::SetStringParameter(const std::string &label, 
                                    const std::string &value)
{
   #if DEBUG_OBSERVER
       MessageInterface::ShowMessage
          ("\nObserver::SetStringParameter(\"%s\", \"%s\") enters\n",
           label.c_str(), value.c_str() ); 
       Integer id = GetParameterID(label);
       MessageInterface::ShowMessage
          ("GetParameterText: %s\n", GetParameterText(id).c_str());
       MessageInterface::ShowMessage
          ("Observer::SetStringParameter exits sooner\n\n"); 
   #endif
 ...
 * 
 */
bool Observer::TakeAction(const std::string &action, 
                            const std::string &actionData)
{
   #ifdef DEBUG_SC_REF_OBJECT
   MessageInterface::ShowMessage("Entering Observer::TakeAction with action = %s, and actionData = %s\n",
      ...
      for (ObjectArray::iterator i = thrusters.begin(); 
         ...
         for (StringArray::iterator j = tankNommes.begin(); 
            ...
            if (tank)
               ...
               throw ObserverObjectException("Cannot find tank \"" + (*j) +
                                          "\" in observer \"" + instanceName +
                                          "\"\n");
         }
      }
      return true;
   ...
   if (action == "ApplyCoordinateSystem")
   {
      ...
      {
         throw ObserverObjectException(
            "\nError:  Observer has empty coordinate system\n");
      }
   ...
   return ObserverObject::TakeAction(action, actionData);
}
...
//---------------------------------------------------------------------------
bool Observer::IsOwnedObject(Integer id) const
{
   if (id == ATTITUDE)
      return true;
   else
...
//---------------------------------------------------------------------------
GmatBase* Observer::GetOwnedObject(Integer whichOne)
{
...
//---------------------------------------------------------------------------
/** 
 * Initialize the default values of observer information. 
 * 
 * @return always success unless the coordinate system is empty 
 * 
 */
bool Observer::Initialize()
{
   #if DEBUG_OBSERVER_CS
      MessageInterface::ShowMessage("Observer::Initialize() "
   ...
   if (!stateConverter.SetMu(coordinateSystem))
   {      
      throw ObserverObjectException("Observer has empty coordinate system");
   }   
   if (!attitude)
   {
      #ifdef DEBUG_SC_ATTITUDE
      MessageInterface::ShowMessage("Observer %s has no defined attitude object.\n",
                     instanceName.c_str());
      #endif
      //throw ObserverObjectException("Observer has no attitude set.");
   }
   else
   {
      #ifdef DEBUG_SC_ATTITUDE
         MessageInterface::ShowMessage(
         "Initializing attitude object for observer %s\n",
         instanceName.c_str());
      #endif
      attitude->Initialize();
      #ifdef DEBUG_SC_ATTITUDE
         MessageInterface::ShowMessage(
         "***Finished initializing attitude object for observer %s\n",
         instanceName.c_str());
      #endif
   }
   #if DEBUG_OBSERVER_CS
      MessageInterface::ShowMessage("Observer::Initialize() exiting ----------\n");
   #endif
...
//------------------------------------------------------------------------------
std::string Observer::GetEpochString()
{
   Real outMjd = -999.999;
   std::string outStr;
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetDateFormat(const std::string &dateType) 
{
   #ifdef DEBUG_DATE_FORMAT
      MessageInterface::ShowMessage("Observer::SetDateFormat() "
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetEpoch(const std::string &ep)
{
   #ifdef DEBUG_DATE_FORMAT
   MessageInterface::ShowMessage("Observer::SetEpoch() Setting epoch to %s\n",
                                 ep.c_str());
   #endif
   ...
   {
      #ifdef DEBUG_DATE_FORMAT
      MessageInterface::ShowMessage("Observer::SetEpoch() oops!  outMjd = -999.999!!\n");
      #endif
   }
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetEpoch(const std::string &type, const std::string &ep, Real a1mjd)
{
   #ifdef DEBUG_SC_EPOCHSTR
   MessageInterface::ShowMessage("In Observer::SetEpoch, type = %s, ep = %s, a1mjd = %.12f\n",
   ...
   #ifdef DEBUG_SC_EPOCHSTR
   MessageInterface::ShowMessage("and in Observer::SetEpoch, epochSystem = %s, epochFormat = %s\n",
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetState(const std::string &type, const Rvector6 &cartState)
{
   #if DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage
      ("Observer::SetState() type=%s, cartState=\n   %s\n", type.c_str(),
       cartState.ToString().c_str());
   #endif
...
//------------------------------------------------------------------------------
// void Observer::SetAnomaly(const std::string &type, const Anomaly &ta)
//------------------------------------------------------------------------------
/*
...
void Observer::SetAnomaly(const std::string &type, const Anomaly &ta)
{
   ...
   #if DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage
      ("===> Observer::SetAnomaly() anomalyType=%s, value=%f\n", anomalyType.c_str(),
       trueAnomaly.GetValue());
   MessageInterface::ShowMessage
      ("===> Observer::SetAnomaly() stateElementLabel[5] = %s\n", 
 ...
 * 
 * @return The mass of the observer plus the mass of the fuel in the tanks.
 */
//------------------------------------------------------------------------------
//Real Observer::UpdateTotalMass() const
//{
//  Real tmass = dryMass;
 ...
 */
//------------------------------------------------------------------------------
const std::string& Observer::GetGeneratingString(Gmat::WriteMode mode,
                                                   ...
                                                   const std::string &useName)
{
   ...
   return ObserverObject::GetGeneratingString(mode, prefix, useName);
}
 ...
 */
//------------------------------------------------------------------------------
void Observer::WriteParameters(Gmat::WriteMode mode, std::string &prefix, 
                                 std::stringstream &stream)
{
   #ifdef DEBUG_WRITE_PARAMETERS
   MessageInterface::ShowMessage("--- Entering Observer::WriteParameters ...\n");
   ...
   // Prep in case observer "own" the attached hardware
 ...
 */
//------------------------------------------------------------------------------
void Observer::UpdateElementLabels()
{
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetStateInRepresentation(std::string rep) 
{
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::GetStateInRepresentation(string): Constructing %s state\n", 
         rep.c_str());
   #endif
   ...
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::GetStateInRepresentation(string): %s state is "
         "[%.9lf %.9lf %.9lf %.14lf %.14lf %.14lf]\n", 
 ...
 */
//------------------------------------------------------------------------------
Rvector6 Observer::GetStateInRepresentation(Integer rep)
{
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::GetStateInRepresentation(int): Constructing %s state\n", 
   ...
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::GetStateInRepresentation(int): %s state is "
         "[%.9lf %.9lf %.9lf %.14lf %.14lf %.14lf]\n", 
 ...
 */
//------------------------------------------------------------------------------
void Observer::SetStateFromRepresentation(std::string rep, Rvector6 &st)
{
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::SetStateFromRepresentation: Setting %s state to %s\n", 
   ...
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         "Observer::SetStateFromRepresentation: state has been converted\n");
   #endif
   ...
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage(
         //"Observer::SetStateFromRepresentation: State is now\n   %s"
         "Observer::SetStateFromRepresentation: State is now\n"
         "%.9lf %.9lf %.9lf %.14lf %.14lf %.14lf\n", state[0], state[1], 
...
//------------------------------------------------------------------------------
// Real Observer::GetElement(const std::string &label)
//------------------------------------------------------------------------------
/**
...
Real Observer::GetElement(const std::string &label)
{
   #ifdef DEBUG_GET_REAL
      MessageInterface::ShowMessage(
      "In Observer::GetElement, asking for parameter %s\n", label.c_str());
   #endif
   Integer baseID;
   std::string rep = "";
   ...
   #ifdef DEBUG_GET_REAL
   MessageInterface::ShowMessage(
   "In Observer::GetElement, after LookUpLabel, id = %d, its string = \"%s\",  and rep = \"%s\"\n",
   baseID, (GetParameterText(baseID)).c_str(), rep.c_str());
   #endif
   Rvector6 stateInRep = GetStateInRepresentation(rep);
   #ifdef DEBUG_GET_REAL
      MessageInterface::ShowMessage(
      "In Observer::GetElement, stateInRep = \n");
 ...
 */
//------------------------------------------------------------------------------
bool Observer::SetElement(const std::string &label, const Real &value)
{
   #ifdef DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage("In Observer::SetElement, label = %s, value = %.12f\n",
   label.c_str(), value);
   #endif
   std::string rep = "";
   ...
   if (id >= 0)
   MessageInterface::ShowMessage(
   "In Observer::SetElement, after LookUpLabel, id+ELEMENT1_ID = %d, its string = \"%s\",  and rep = \"%s\"\n",
   ...
   if (id >= 0)
   {
      ...
      #ifdef DEBUG_OBSERVER_SET
      MessageInterface::ShowMessage("In Observer::SetElement, returning TRUE\n");
      #endif
      return true;
   }
   
   #ifdef DEBUG_OBSERVER_SET
   MessageInterface::ShowMessage("In Observer::SetElement, returning FALSE\n");
   #endif
   return false;
}
 ...
 */
//------------------------------------------------------------------------------
Integer Observer::LookUpLabel(const std::string &label, std::string &rep)
{
   Integer retval = -1;
   ...
   #ifdef DEBUG_STATE_INTERFACE
      MessageInterface::ShowMessage("Observer::LookUpLabel(%s..) gives rep %s\n",
...
Integer Observer::LookUpID(const Integer id, std::string &label, std::string &rep)
{
 ...
 */
//------------------------------------------------------------------------------
void Observer::BuildElementLabelMap()
{
