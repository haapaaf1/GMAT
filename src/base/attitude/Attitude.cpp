...
{
   refCSName = refName;
   isInitialized = false; // need to reinitialize
   return true;
}
   ...
   if (obj->IsOfType("CoordinateSystem"))
   {
      if (name == refCSName)
      {
         ...
         if (refCS != (CoordinateSystem*) obj)
         {
            ...
            isInitialized = false;  // need to reinitialize, since CS has changed
         }
      }
      #ifdef DEBUG_REF_SETTING
      MessageInterface::ShowMessage
...
Real Attitude::GetRealParameter(const Integer id) const
{
   ...
   if (!isInitialized) (const_cast<Attitude*>(this))->Initialize();
                                ...
                                const Real value)
{
   ...
   if (id == EPOCH)  // this should be an A1Mjd time
   {
      if (epoch != value)
      {
         isInitialized = false;  // not really true for CSFixed or SpiceAttitude, though
         epoch         = value;
      }
      return epoch;
   }
