...
//------------------------------------------------------------------------------
/**
 *  Returns a random deviate from the Cauchy distribution
 *  using transformed rejection with decomposition by Wolfgang Hormann.
 *
 *  Hormann, Wolfgang and Derflinger, Gerhard. "The Tranformed Rejection Method
 *  for Generating Random Variables, an Alternative to the Ration of Uniforms
 ...
 * needs to be tested thoroughly for proper statistics
Real RandomNumber::Cauchy(Real x)
{
    const Real ur = 0.5;
    const Real vr = 0.8284264502;
    const Real a = 0.306327;
    const Real b = 1.479078;
    const Real alpha = 0.9623546527;
    const Real alpha_pi_inv = alpha/GmatMathUtil::PI;
	...
	else
	{
	    ...
	    if ((V*(1+x*x) - b*alpha_pi_inv)*
		(0.25-GmatMathUtil::Abs(U)+U*U) <= a*alpha_pi_inv)
	    {
		return gu;
	    }
	    else
 ...
 *  Method." Communications in Statistics: Simulation and Computation 23, 3, 1994.
 *
 *  @param <x0> location parameter specifying the peak
 *  @param <gamma> scale parameter equivalent to the interquartile range
 *  @return The random deviate.
 *
 */
//------------------------------------------------------------------------------
/* Cauchy distribution not ready for prime time
 * needs to be tested thoroughly for proper statistics
Real RandomNumber::Cauchy(Real x, Real x0, Real gamma)
{
    const Real ur = 0.5;
    const Real vr = 0.8284264502;
    const Real a = 0.306327;
	...
	else
	{
	    ...
	    if ((V*gamma*(1+x1*x1) - b*alpha_pi_inv)*
		(0.25-GmatMathUtil::Abs(U)+U*U) <= a*alpha_pi_inv)
	    {
		return gu;
	    }
 ...
 * needs to be tested thoroughly for proper statistics
Real RandomNumber::Poisson(Real mu)
{
    // These approximations are only valid for mu >= 10
    if (mu >= 10) return 0.0;
    
    //const Real c = mu + 0.445;
    const Real b = 0.931+2.53*GmatMathUtil::Sqrt(mu);
    const Real a = -0.059 + 0.2483*b;
    const Real ur = 0.43;
    const Real vr = 0.9277 - 3.6224/(b-2.0);
    const Real alpha_inv = 1.1239 + 1.1328/(b-3.4);
	...
	else
	{
	    ...
	    if (GmatMathUtil::Ln(V*alpha_inv/(a/(0.25-
		    GmatMathUtil::Abs(U)+U*U))+b*b)
		 <= -mu + GmatMathUtil::Floor(gu)*GmatMathUtil::Ln(mu)
		    - GmatMathUtil::Ln(Factorial(GmatMathUtil::Floor(gu))))
	    {
		return gu;
	    }
	    else
...
//------------------------------------------------------------------------------
/**
 *  Computes the factorial of the integer n. This is exact for n <= 10.
 *  For n > 10, Stirling's approximation is used.
 *
 *  @param <n> Integer for which the factorial is desired
 *  @return The resulting factorial
 *
 */
//------------------------------------------------------------------------------
/* Required for Poisson distribution
Real RandomNumber::Factorial(Integer n)
{
    if (n == 0)
    {
	return 1.0;
	...
	return GmatMathUtil::Sqrt(GmatMathUtil::TWO_PI*n)*
	GmatMathUtil::Pow(n/GmatMathUtil::E,n);
    }
				          ...
				          const Real a, const Real b)
{
     ...
     dsfmt_fill_array_open_closed(&dsfmt,(double*)myArray,size);
     for (Integer i=0; i < size; i++)
     {
         myArray[i] = a + (b - a)*myArray[i];
     }
