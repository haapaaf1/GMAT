...
bool ProcessB3Data::Initialize()
{
    ...
    if (pcrecpp::RE("^[Rr].*").FullMatch(readWriteMode))
    {
        ...
        i = b3Data.begin();
            ...
            for (std::vector<B3Obtype*>::const_iterator j=b3Data.begin(); j!=b3Data.end(); ++j)
            {
 ...
 */
//------------------------------------------------------------------------------
bool ProcessB3Data::GetData(B3Obtype *myb3Data)
{
 ...
 */
//------------------------------------------------------------------------------
bool ProcessB3Data::WriteData(B3Obtype *myB3Data)
{
 ...
 */
//---------------------------------------------------------------------------
bool ProcessB3Data::IsParameterRequired(const Integer id) const
{
   if (id == NUMLINES_ID)  return true;
   if (id == FILEFORMAT_ID)  return true;
   return GmatBase::IsParameterReadOnly(id);
 ...
 */
//---------------------------------------------------------------------------
bool ProcessB3Data::IsParameterRequired(const std::string &label) const
{
   return IsParameterReadOnly(GetParameterID(label));
}
