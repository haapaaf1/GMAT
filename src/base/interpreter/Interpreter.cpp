...
//------------------------------------------------------------------------------
/**
 *  @note This code BREAKS the generic feature of command parsing, and must be
 *  rewritten to restore the ability to write and add general purpose commands
 *  without overriding the InterpretAction method.
 */
                                        ...
                                        const std::string &value, GmatBase *fromObj)
{
   ...
   else if (pmType == "PrimaryBodies" || pmType == "PointMasses")
   {
      ...
      for (UnsignedInt i=0; i<bodies.size(); i++)
      {
         ...
         if (pm)
         {
            ////pm->SetName(bodies[i]); // It is confusing if body name is set
